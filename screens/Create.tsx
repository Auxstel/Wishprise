...
import { saveSurprise, generateId } from '../services/storageService';
...
export const Create: React.FC = () => {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>, field: 'songUrl' | 'voiceMessageUrl') => {
    const file = e.target.files?.[0];
    if (!file) return;

    // Increased limit to 10MB because we are now using IndexedDB
  ...
  const startRecording = async () => {
    try {
      ...
      mediaRecorder.onstop = () => {
        // Create blob without forcing type first, let browser decide based on chunks
        const audioBlob = new Blob(audioChunksRef.current, { type: mimeType });
        const reader = new FileReader();
        reader.readAsDataURL(audioBlob);
        reader.onloadend = () => {
            const base64data = reader.result as string;
            setFormData(prev => ({ ...prev, voiceMessageUrl: base64data }));
        };
  ...
  const handleSubmit = async () => {
    ...
    try {
        ...
        alert(e.message || "Error saving surprise.");
        setIsSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex flex-col font-sans overflow-hidden">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
          ...
          {/* We use a slight overlay to ensure text contrast against the bright balloons */}
            ...
            <div className="bg-slate-900/80 backdrop-blur-xl p-6 rounded-2xl border border-white/10 space-y-6 shadow-xl overflow-y-auto max-h-[40vh] no-scrollbar">
                ...
                <div>
                    ...
                    {Object.values(CakeStyle).map(style => (
                        ...
                        className={`p-3 rounded-lg border text-[10px] md:text-xs font-medium capitalize transition-all ${
                            formData.cakeStyle === style 
                ...
                <div>
                    ...
                    {Object.values(CakeFlavor).map(flavor => (
                        ...
                        className={`p-2 rounded-lg border text-[10px] md:text-xs font-medium capitalize transition-all ${
                            formData.cakeFlavor === flavor 
                            ? 'border-magical-500 bg-magical-500/20 text-white shadow-lg shadow-magical-500/20' 
                            : 'border-white/5 bg-white/5 text-gray-400 hover:bg-white/10'
                        }`}
                        >
                    ...
                    {!formData.voiceMessageUrl ? (
                        <div className="flex gap-2">
                            <button 
                                ...
                                className={`flex-1 p-3 rounded-lg font-bold text-sm transition-all flex items-center justify-center gap-2 ${
                                    isRecording 
